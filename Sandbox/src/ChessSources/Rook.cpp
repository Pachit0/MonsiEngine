...
int **Rook::move(int coordinateX, int coordinateY)
{
	...
	for (int i = coordinateX, int j = 0; i < MAX_COORD; i++, j++)
		possibleMoves[j][0] = i - 1 + (coordinateY - 1) * 8;

	for (int i = coordinateY, int j = 0; i < MAX_COORD; i++, j++)
		possibleMoves[j][1] = coordinateX - 1 + (i - 1) * 8;

	for (int i = coordinateX, int j = 0; i > MIN_COORD; i--, j++)//moje da se naloji naobratno da se zapisvat v masiva, ne ot nai golemi koordinati, za blockirani figuri ot drugi e vajno
		possibleMoves[j][2] = i - 1 + (coordinateY - 1) * 8;

	for (int i = coordinateY, int j = 0; i > MIN_COORD; i--, j++)
